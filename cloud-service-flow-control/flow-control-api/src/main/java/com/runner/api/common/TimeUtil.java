...
import com.runner.misc.utils.Tuple2;
...
public final class TimeUtil implements Runnable {
    ...
    private AbstractSlidingWindow<Statistic> statistics;
    ...
    public TimeUtil() {
        this.statistics = new AbstractSlidingWindow<Statistic>(3, 3000) {
            ...
            @Override
            protected BucketWrapper<Statistic> resetBucketTo(BucketWrapper<Statistic> windowWrap, long startTime) {
    ...
    public void run() {
        while (true) {
            ...
            if (this.state == STATE.RUNNING) {
                this.currentTimeMillis = System.currentTimeMillis();
                this.statistics.currentBucket(this.currentTimeMillis).value().getWrites().increment();
                try {
...
//                RecordLog.debug("TimeUtil switches to RUNNING");
    ...
    public Tuple2<Long, Long> currentQps(long now) {
        List<BucketWrapper<Statistic>> list = this.statistics.listAll();
        long reads = 0;
        long writes = 0;
        int cnt = 0;
        for (BucketWrapper<Statistic> windowWrap : list) {
            if (windowWrap.isTimeInBucket(now)) {
                continue;
            }
            cnt++;
...
//            RecordLog.info("TimeUtil switches to IDLE due to not enough load, reads={}/s, writes={}/s", qps.r1, qps.r2);
    ...
    private long currentTime(boolean innerCall) {
        ...
        Statistic val = this.statistics.currentBucket(now).value();
        if (!innerCall) {
